...
- **The station's own logo**, leading, 40 dp, square, clipped to a 6 dp corner and scaled to *fit*. Bundled in the APK (`StationLogos`), not fetched: the station table is a constant, so a logo has nothing to discover, and downloading five known images would buy an image-loading dependency, a cache, a placeholder state and a first draw after a cold boot that is worse than the second. Scaled to fit rather than cropped because three of the five are wordmarks — "radio FM4" is *text*, and a crop that shaves a letter off is worse than empty space beside a round mark. A station with no logo falls back to the generic radio glyph; **no house-style placeholder**, the same rule the title line follows.
...
- **The logo and the two text lines are one tap target, and it opens the station picker.** A dropdown of the whole table, each row a logo and a display name, the running station marked with a check and still tappable — a re-tune is the cheapest way to restart a stream that has gone strange, and a list whose rows move depending on what is on is a bad list to operate from muscle memory at arm's length. Picking a station calls `RadioSock.playFromPanel(station)`, which is the identical `tuneTo` that `radio.play_radio` performs: same focus claim, same buffering spinner, same reconnect ladder, same `Error` card if it never connects. **The X is deliberately outside the tap target** — "switch station" and "turn it off" are opposite intentions, and one target that could be either is wrong half the time.
